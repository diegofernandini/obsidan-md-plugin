...
- **PD**
[[sample_banca_reporte.pdf]]: DOI: 10.5281/zenodo.10785173 
952 | V 1 9 . I 0 2  
Aquí utilizaremos notación Dirac para expresar un qubit en superposición de estados [6]; véase ecuación:
...
> Rastrea la posición original a través de un mapa fuente.

`trace-mapping` permite seguir el código de una salida y rastrearlo hasta la ubicación original en el archivo fuente a través de un mapa fuente.

Esta herramienta se puede usar para obtener la línea y columna originales en el código fuente dado la posición en el archivo compilado. Puede utilizarla para implementar funcionalidades como depuración, seguimiento de errores o incluso para mejorar la experiencia del usuario al proporcionar información más detallada sobre las fuentes.
...
// Las líneas comienzan en la línea 1, las columnas en el columna 0.
const rastreada = originalPositionFor(tracer, { line: 1, column: 5 });
assert.deepEqual(rastreada, {
  fuente: 'input.js',
  línea: 42,
  columna: 4,
  nombre: 'foo',
...
const generada = generatedPositionFor(tracer, {
  fuente: 'input.js',
  línea: 42,
  columna: 4,
});
assert.deepEqual(generada, {
  línea: 1,
  columna: 5,
});
```
...
Se proporciona una API más baja para obtener el segmento que coincide con nuestra línea y columna. A diferencia de `originalPositionFor`, `traceSegment` utiliza un 0-base para `linea`:

```typescript
import { traceSegment } from '@jridgewell/trace-mapping';

// La línea es 0-base.
const rastreada = traceSegment(tracer, /* línea */ 0, /* columna */ 5);

// Los segmentos son [outputColumn, sourcesIndex, sourceLine, sourceColumn, namesIndex]
// Una vez más, la línea es 0-base y también lo es sourceLine
assert.deepEqual(rastreada, [5, 0, 41, 4, 0]);
```

**Sourcemaps Segmentadas**

El spec de sourcemaps define un campo especial llamado `sections` que está diseñado para manejar la concatenación del código de salida con mapas fuente asociados. Este tipo de mapa fuente es poco común (ninguna herramienta principal produce), pero si estás codificando una concatenación a mano, puede ser necesario. Se proporciona un helper `AnyMap` que puede recibir tanto un mapa fuente regular como un mapa fuente segmentado y devuelve un objeto `TraceMap`:

```typescript
import { AnyMap } from '@jridgewell/trace-mapping';
const fooOutput = 'foo';
const barOutput = 'bar';
const salida = [fooOutput, barOutput].join('\n');

const segmentado = new AnyMap({
  version: 3,
  secciones: [
    {
      // La línea y la columna son 0-base
      offset: { line: 0, column: 0 },
      // El mapa fuente de fooOutput
      map: {
        version: 3,
        fuentes: ['foo.js'],
        nombres: ['foo'],
    ...
    {
      // El mapa fuente de barOutput no afectará la primera línea, solo la segunda
...
const rastreada = originalPositionFor(segmentado, {
  línea: 2,
  columna: 0,
});
...
Más rápido es trace-mapping:    decoded Object input
...
Más rápido es trace-mapping:    decoded originalPositionFor
