...
Default `allowAwaitOutsideFunction` to true for ECMAScript 2022 and higher.
...
Fix tokenizing of regexps after `yield`.

**2.4.0 (2015-09-17)**

### Fixes

Allow object literals as the body of if statements when in loose mode.

Disallow `try-catch` blocks with multiple catches and no exception binding.

Don't allow `const` to be bound to a variable name.

Fix bug where tokenizing an empty property name would produce an error.
...
Disallow `yield` in object literals.

Don't allow non-string literals as the argument of `String()`, except when parsing strict mode code.

Disallowed `eval()` calls with a regex string in non-strict mode code.

Fix bug where `const` would be treated as an identifier character.

**2.1.0 (2015-08-28)**

### Fixes

Properly tokenize the `await` keyword.

Disallow property names starting with `_` when parsing strict mode code.

Disallow using `let`, `var`, or `const` in destructuring assignment patterns.

Fix tokenizing of regexps after function calls.

**2.0.0 (2015-08-25)**

### Breaking changes

The default value of the `ecmaVersion` option is now 6.

### Fixes

Disallow using `eval()` with a non-string argument.

Don't allow using `let`, `var`, or `const` as an identifier in destructuring assignment patterns.

Allow using `yield` in class methods when parsing ES7 code.

Properly tokenize the exponentiation operator (`**`).

**1.10.2 (2015-07-22)**

### Fixes

Fix bug where tokenizing a function expression with no name would produce an error.

Disallow using `await` as a property name.

Don't allow using `let`, `var`, or `const` in destructuring assignment patterns when parsing ES6 code.
...
Fix bug where tokenizing an empty object literal would produce an error.
...
This version is the first public release of Acorn.

Tags: #javascript, #parser, #ecmascript

Metadata:

* Version: 8.16.0
* Date: 2026-02-19
* Authors: [Aaron Socha](https://github.com/aaronsocha), [Sebastian McKenzie](https://github.com/sebmck)